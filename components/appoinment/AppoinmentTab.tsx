...
export const AppointmentTab: React.FC = () => {
  ...
  const handleSubmit = (data: AppointmentFormData) => {
    if (isEditMode && currentAppointment) {
      updateAppointment(currentAppointment.id, data);
    } else {
      addAppointment(data);
    }
  ...
  return (
    <View className="flex-1">
      <View className="flex-row justify-between items-center mb-4">
        <Text className="text-xl font-bold text-gray-800">Doctor Appointments</Text>
        <TouchableOpacity 
      ...
      <ScrollView className="flex-1">
        {appointments.length > 0 ? (
          ...
          <Text className="text-gray-500 text-center py-6">No appointments scheduled.</Text>
        )}
      </ScrollView>
